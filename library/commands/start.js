...
export default async (argv) => {
  ...
  const baseDirectory = argv.where || ".";

  if (!baseDirectory)
    console.warn("No base directory provided, using current directory.");

  // check for config file in the base directory
  if (!readdirSync(baseDirectory).includes("config.js"))
    throw new Error("No config file found! Please use `init` to create one.");
  ...
  if (readdirSync(baseDirectory).includes("commands")) {
    ...
    client.on("ready", async () => {
      ...
      readdirSync(`${baseDirectory}/commands/global`).forEach(async (file) => {
        if (!file.endsWith(".js")) return;

        const command = (
          await import(`./${baseDirectory}/commands/global/${file}`)
        ).default;
        ...
        if (command.name && command.description && command.execute) {
          ...
          console.log(
            `❌ ./${baseDirectory}/commands/global/${file} is not a valid global command file!`
          );
        }
      });
      ...
      readdirSync(`${baseDirectory}/commands/guilds`).forEach(async (guild) => {
        readdirSync(`${baseDirectory}/commands/guilds/${guild}`).forEach(
          async (file) => {
            ...
            const command = (
              await import(
                `./${baseDirectory}/commands/guilds/${guild}/${file}`
              )
            ).default;
